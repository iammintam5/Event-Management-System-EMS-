...
if (directUrl) {
  ...
  if (url.searchParams.has('schema') && url.searchParams.get('schema') !== 'event') {
    throw new Error('DIRECT_URL must target the event schema');
  }
  url.searchParams.set('schema', 'event');
