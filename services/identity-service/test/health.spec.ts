...
describe('identity service foundation', () => {
  it('boots the health controller and reports its own service identity', async () => {
    ...
    expect(module.get(HealthController).check()).toEqual({ service: 'identity', status: 'up' });
    await module.close();
  });
  ...
  it('rejects a connection configured for another service schema without leaking credentials', () => {
    ...
    expect(() => validateEnvironment(env)).toThrow('DATABASE_URL must target the identity schema');
  });
});
