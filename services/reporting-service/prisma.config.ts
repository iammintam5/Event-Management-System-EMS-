...
if (directUrl) {
  ...
  if (url.searchParams.has('schema') && url.searchParams.get('schema') !== 'reporting') {
    throw new Error('DIRECT_URL must target the reporting schema');
  }
  url.searchParams.set('schema', 'reporting');
