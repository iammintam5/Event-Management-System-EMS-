...
if (directUrl) {
  ...
  if (url.searchParams.has('schema') && url.searchParams.get('schema') !== 'registration') {
    throw new Error('DIRECT_URL must target the registration schema');
  }
  url.searchParams.set('schema', 'registration');
